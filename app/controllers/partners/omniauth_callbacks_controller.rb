module Partners
  class OmniauthCallbacksController < Devise::OmniauthCallbacksController
    def pro_sante_connect
      psc_service = OmniauthProSanteConnectService.new(request.env["omniauth.auth"]["extra"]["raw_info"])
      try_login_or_link(psc_service)
    end

    def bimedoc
      bimedoc_service = OmniauthBimedocService.new(request.env["omniauth.auth"]["extra"]["raw_info"])
      try_login_or_link(bimedoc_service)
    end

    protected

    def try_login_or_link(service)
